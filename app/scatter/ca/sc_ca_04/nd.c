...
static const uint16_t BEACON_INTERVAL = 328; //10ms = 327,68 ticks
static const uint8_t BEACON_DURATION = 9; //0.258 ms = 8,45 ticks
//radio utilisation time for beacon transmission: 576us = 18,87 ticks = 19 ticks.
static uint16_t SLOT_DURATION;
...
static const uint8_t MAX_SLACK = 66; //2 ms = 65.54 ticks
#endif
/*---------------------------------------------------------------------------*/
...
nd_start(uint8_t mode, const struct nd_callbacks *cb){
  ...
  RX_DURATION = BEACON_INTERVAL + BEACON_DURATION;
  #if CA
  RX_DURATION += (2*MAX_SLACK);
  #endif
  
  PRINTF("DEBUG: the following duration will be used:\n Beacon Duration: %u\n  Beacon Interval: %u\n  RX duration: %u\n",BEACON_DURATION,BEACON_INTERVAL,RX_DURATION);
  ...
  *** END BUG **/
  ...
  /***********************************/
  //Initialize the various durations. This cannot be done outside because some sort of bug
...
void end_tx_slot(struct rtimer *t, uint8_t *mode){
  ...
  /** Collision avoidance **/
  if(*mode == ND_BURST){
    //Reset counter only if in BURST since in SCATTER we need it across slots
    rnd_counter = 0;
  }
  /** **/
  #endif
